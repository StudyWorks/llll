...
public class HtmlBuilder {
	...
	/**
	 * <p>
	 * The padding-left attribute [cellpadding-left=].
	 * </p>
	 */
	public HtmlBuilder paddingLeft(String paddingLeft) {
		if (StringUtils.isNotBlank(paddingLeft)) {
			append(" padding-left=\"").append(paddingLeft).append("\" ");
		}
	 ...
	 *            The text to display for the url.
	 * @param params
	 *            A Map of name/value pair of parameters.
	 */
	public HtmlBuilder ahref(String url, String displayText,
		...
		a().href().quote().append(urlBuilder.toString()).quote().close()
				.append(displayText).aEnd();
		return this;
	}
	...
	public HtmlBuilder option(Object value, Object displayName,
			Object currentValue) {
		String strValue = value.toString().trim();
		this.option().value(strValue);
		if (null != currentValue
				&& currentValue.toString().trim().equals(strValue)) {
			this.selected();
		}
		this.close().append(displayName).optionEnd();
		return this;
	}

	public HtmlBuilder newLimitVar(String ctx, boolean useAjax) {
		this.script().type("text/javascript").end();
		this.append("$(function(){ECUtils.tablify('" + ctx + "', " + useAjax + ");" + "});");
		this.scriptEnd();
		return this;
	}

	public HtmlBuilder onchange(String functionName, Object... args) {
		return this.event("onchange", functionName, args);
	}

	public HtmlBuilder onclick(String functionName, Object... args) {
		return this.event("onclick", functionName, args);
	}

	public HtmlBuilder event(String eventName, String functionName,
			Object... args) {
		return append(" " + eventName + "=\"").function(functionName, args)
				.append("\" ");
	}

	public HtmlBuilder function(String name, Object... args) {
		String[] jsArgs = new String[args.length];
		for (int i = 0; i < args.length; i++) {
			jsArgs[i] = args[i].toString();
